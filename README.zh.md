...
使用本 skill 的 agent 会优先遵循用户显式指定的模板；如果请求只用自然语言
描述评审目标，agent 会读取可信模板的作者注释并选择语义最匹配的一项，意图
不明确时回退到 `default`。例如，要求质疑不必要的抽象或复杂度时，会选择
`avoid-overengineering`，用户无需知道模板文件名。
